...
function generateTexts() {
    let resultText = `Datos persona nueva\n`;
    resultText += `Fecha: ${generalInfo[0]}\n`;
    resultText += `CELEBRACION: ${generalInfo[1]}\n\n`;

    responses.forEach((response, index) => {
        resultText += `NOMBRES: ${response[0]}\n`;
        resultText += `CELULAR: ${response[3]}\n`;
        resultText += `INVITADO POR: ${response[1]}\n`;
        resultText += `OBSERVACION: ${response[2]}\n\n`;
    });
